...
import { SandwichesResponse, GarnitureResponse, SaucesResponse } from "../../../types.ts";
...
import AddSandwichNotification from "../../snackbar_creerSandwich/AddSandwichNotification.tsx";
...
export default function Sandwichs() {
  ...
  const [currentSandwich, setCurrentSandwich] = useState<SandwichesResponse | null>(null);
  ...
  const toggleSauces = (sauce: SaucesResponse) => {
    ...
    setSelectedSauces((prev) =>
        prev.includes(sauce.nom)
            ? prev.filter((name) => name !== sauce.nom)
            : [...prev, sauce.nom]
    );
  };
  ...
  const renderModalContent = () => (
      <>
        <DialogTitle sx={{ mt: 1, fontFamily: "cursive", fontWeight: "bold" }}>Type de sandwich : {currentSandwich?.nom}</DialogTitle>
                ...
                {garnitureData?.map((item: GarnitureResponse) => (
                    <ListItem key={item.nom} component="li" disablePadding>
                      ...
                      >
                        ...
                        <ListItemText
                            primary={item.nom}
                            secondary={item.disponible ? "Disponible" : "Non disponible"}
                        />
                      </ListItemButton>
                    </ListItem>
                ))}
      ...
      <>
        <Grid container spacing={4} justifyContent="center" alignItems="center">
          {data?.map((sandwich: SandwichesResponse) => (
              ...
              >
                <Card sx={{ maxWidth: 345, border: "none", backgroundColor: "transparent", transition: "background-color 0.3s ease",
                  ...
                  <CardActionArea onClick={() => handleCardClick(sandwich)}>
                    ...
                    <CardContent>
                      ...
                      <Typography
                          ...
                          sx={{ fontFamily: "cursive" }}
                      >
                        {sandwich.disponible ? "Disponible" : "Non disponible"} - {sandwich.prix} €
                      </Typography>
                    </CardContent>
                  </CardActionArea>
                </Card>
