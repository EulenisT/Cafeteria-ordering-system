...
import { clearCart, removeFromCart, setBalanceUser, setCurrentUser } from "../../../../store/expense/expense-slice";
...
export function Panier() {
    ...
    const { currentUser, carts, balanceUser } = useSelector((state: RootState) => state.EXPENSE);
    const personalizedSandwiches =
        currentUser && carts[currentUser] ? carts[currentUser].personalizedSandwiches : [];

    const { data: userProfile, isLoading, isSuccess } = useQuery({
        queryKey: ["userInfo"],
        queryFn: getUserInfo,
        initialData: {
    ...
    return (
        <Box sx={{ width: "100%", mt: 4, px: { xs: 2, md: 4 }, mb: "80px" }}>
            ...
            <Box sx={{ textAlign: "center", mb: 2 }}>
                {isLoading ? (
                    <Typography variant="h6">Chargement du solde...</Typography>
                ) : (
                    <Typography variant="h6">Solde: {balanceUser.toFixed(2)} €</Typography>
                )}
            </Box>
