    ...
    return (
        <Box sx={{ display: "flex", flexDirection: "column" }}>
            ...
            <Box sx={{ paddingBottom: "25px" }}>
                <List>
                    {garnitureData?.map((itemGarniture: GarnitureResponse, index: number) => (
                        <React.Fragment key={itemGarniture.nom}>
                            <ListItem
                                alignItems="flex-start"
                                style={{
                                    position: "relative",
                                    backgroundColor: selectedGarnitures.includes(itemGarniture.nom)
                                        ? "#F2D4D6"
                                        : hoveredGarnitureIndex === index && itemGarniture.disponible
                                            ? "#f9e1e6"
                                            : "transparent",
                                ...
                                onClick={() =>
                                    itemGarniture.disponible && handleAddClickGarniture(itemGarniture)
                                }
                            >
                                <ListItemAvatar>
                                    <Avatar alt={itemGarniture.nom} src="https://via.placeholder.com/40" />
                                </ListItemAvatar>
                                <ListItemText
                                    primary={
                ...
                <List>
                    {saucesData?.map((itemSauce: SaucesResponse, index: number) => (
                        <React.Fragment key={itemSauce.nom}>
                            ...
                            >
                                <ListItemAvatar>
                                    <Avatar alt={itemSauce.nom} src="https://via.placeholder.com/40" />
                                </ListItemAvatar>
                                <ListItemText
                                    primary={
                                        <Typography variant="h6" component="span">
            ...
            {/* Notificación */}
            <AddSandwichNotification open={notificationOpen} onClose={handleNotificationClose} />
        </Box>
    );
}
